...
def main():
    ...
    out=dict(status='stopped_after_failure' if failures else ('complete_conditional_batch_audited' if not missing else 'partial_conditional_batch_audited'),completed=len(rows),expected=60,
        missing=missing,failures=failures,receipts=receipts,effects=effects,plan_sha256=sha(PACKAGE/plan_name),auditor_sha256=sha(Path(__file__).parent/'audit_ab_cb.py'),
    ...
    table_name='AB_CB_paced_periodic_runs.csv' if args.paced else 'AB_CB_periodic_runs.csv'
    if rows:
        raw=[]
    ...
    md=['# AB-CB完整周期通信复核（条件实验，未获安全准入）','',f'已审计 {len(rows)}/60 个具名运行；每格计划5次。n<5只作过程记录，不给置信区间。','',
        ...
        '|---|---|---|---|---:|---:|---:|---:|---|']
    if args.paced:
        md[0]='# AB-CB完整周期通信复核：公开加载节拍修订（条件实验）'
    ...
    (PACKAGE/('28_AB_CB公开加载周期结果.md' if args.paced else '25_AB_CB完整周期条件实验.md')).write_text('\n'.join(md)+'\n',encoding='utf-8')
