...
from common import *
import copy
from run_ab_dummy_first_periodic import run,identity
from audit_ab_dummy_first import audit_record,matched


def main():
    source=identity();proof=sha(PACKAGE/'results/ab_dummy_first_frontend_checks.json');rows=[]
    for bottom,layout in ((None,'uniform'),(0,'bottom_d0')):
        for kind in ('ring','gc_ring','r0'):
            s=dict(id=f'check_ABDF_{layout}_{kind}',kind=kind,N=64,B=64,L=5,X=8,plb=3,llc_sets=4,llc_ways=2,
                ...
                trace_seed=701,oram_seed=1701,workload='hot90',experiment_class='ab_dummy_first_periodic_checks')
    ...
    r=rows[-1];rejected=[]
    edits=[('wrong_selection_policy',lambda x:x.__setitem__('cb_selection_policy','uniform_unused')),('old_initial_clock',lambda x:x['initialization'].__setitem__('initial_clock',73)),
    ...
    save(PACKAGE/'results/ab_dummy_first_periodic_checks.json',dict(status='passed',source_hashes=source,positive_cases=len(rows),negative_checks=rejected,
        auditor_sha256=sha(Path(__file__).parent/'audit_ab_dummy_first.py'),base_auditor_sha256=sha(Path(__file__).parent/'audit_ab_cb.py'),checker_sha256=sha(__file__),
