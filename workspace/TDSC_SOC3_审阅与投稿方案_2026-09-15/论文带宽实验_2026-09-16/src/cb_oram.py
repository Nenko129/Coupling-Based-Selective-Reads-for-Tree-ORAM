...
class Config:
    kind:str; L:int; N:int; B:int=4096; Z:int=4; A:int=3; S:int=3; R:int=192; tree:int=0; compact:bool=True; fused:bool=True; Z_by_depth:tuple=(); S_by_depth:tuple=(); Y:int=0; Y_by_depth:tuple=()
    def __post_init__(self):
        if not self.ring or not self.fused:raise ValueError('CB engine requires a fused Ring-family protocol')
    ...
    def at(self,b):
        return BucketConfig(self,depth(b)) if self.Z_by_depth or self.S_by_depth or self.Y_by_depth else self
    ...
    @property
    def secret(self):return ((8+(8 if self.Y else 0)+(30 if self.compact else 64)*self.Z+31)//32)*32
    @property
    def H(self):return PUBLIC+NONCE+self.secret
    @property
    def W(self):return self.B+NONCE
    def context(self):
        base=encode(b'Selective-ORAM-optimization-v3',self.kind.encode(),*[ui(v,8) for v in (self.tree,self.L,self.N,self.B,self.Z,self.A,self.S,self.R,int(self.compact),int(self.fused))])
...
class Header:
    ...
    green:int=0 # non-target real blocks consumed in this bucket epoch, encrypted
    ...
    def secret_bytes(self,c:Config)->bytes:
        ...
        need(0<=self.green<=c.Y,'green counter bounds')
    ...
    def decode(cls,c:Config,pub:bytes,plain:bytes):
        ...
        r=Cursor(plain);h.green=r.uint(8) if c.Y else 0
        need(0<=h.green<=min(c.Y,h.count),'green counter bounds')
        livebits=r.uint(8);need(livebits<(1<<c.Z),'descriptor bitmap')
        for i in range(c.Z):
...
class Server:
    ...
    def handle(self,raw:bytes)->bytes:
        ...
        if op==INIT:
            b=q.uint();need(1<=b<(1<<(c.L+1)),'init bucket');bc=c.at(b)
            if c.rootless and b==1:
                ...
                local={p:q.take(TAG) for p in local_indices(bc.n)} if c.ring else {}
            ...
            if op in (OPEN_FULL,OPEN_HEAD):
                ...
                for d in ds:
                    bc=c.at(path[d]);inds=from_bitmap(q.uint(),bc.n);s=self.buckets[tree,path[d]]
                    need(bool(inds),'empty subset');out+=b''.join(s.slots[j] for j in inds)
                    out+=b''.join(s.local[p] for p in witnesses(bc.n,inds))
                q.end()
            else:
                fs=q.uint();full=from_bitmap(fs,c.L+1);need(set(full)<=set(ds),'full-write mask')
                for d in ds:
                    b=path[d];bc=c.at(b);old=self.buckets[tree,b];hd=q.take(bc.H)
                    slots=[q.take(c.W) for _ in range(bc.n)] if d in full else old.slots
                    local=({p:q.take(TAG) for p in local_indices(bc.n)} if c.ring else {}) if d in full else old.local
...
class Tree:
    ...
    def prepare(self,b:int,old:Header,records:list[Record]|None,gamma:int|None=None):
        c=self.c.at(b)
        if records is None:
            h=Header(old.gamma,old.w,old.v+1,old.count,old.used,dict(old.live),old.green);slots=None;local=None
        else:
    ...
    def initialize(self):
        ...
        def visit(b:int,d:int):
            ...
            if c.rootless and b==1:
                ...
                body=ui(b,8)+hd+b''.join(slots)+(b''.join(local[p] for p in local_indices(bc.n)) if c.ring else b'')+bt+gt
                comp={'headers_upload':bc.H,'data_upload':bc.n*c.W,'authentication_upload':(len(local)+2)*TAG}
    ...
    def open_path(self,leaf:int,ds:tuple[int,...],stage:str)->View:
        ...
        skip=len(c.levels)-k
        comp={'headers_download':sum(c.at(node(leaf,d,c.L)).H for d in ds),'authentication_download':(c.L+skip+(k if c.ring else 0))*TAG}
        if not c.ring:comp['data_download']=sum(c.at(node(leaf,d,c.L)).n*c.W for d in ds)
        raw=self.io.rpc(c,op,ui(leaf,8)+ui(bitmap(ds),8),stage,{'leaf':leaf,'depths':list(ds)},comp)
        q=Cursor(raw);wh={};D={};cts={};B={}
        for d in ds:
            b=node(leaf,d,c.L);bc=c.at(b);wh[b]=q.take(bc.H)
            if c.ring:D[b]=q.take(TAG)
            else:cts[b]=[q.take(c.W) for _ in range(bc.n)];D[b]=self.droot(b,cts[b])
        ...
        for b,hd in wh.items():
            pub=hd[:PUBLIC];headers[b]=Header.decode(c.at(b),pub,self.P.decrypt(self.ctx,self.oid(b),pub,hd[PUBLIC:]))
    ...
    def read_slots(self,view:View,selection:dict[int,tuple[int,...]],stage:str):
        ...
        proofcount=sum(len(witnesses(c.at(b).n,tuple(sorted(v)))) for b,v in selection.items())
        ...
        q=Cursor(raw);allcts={}
        for d in ds:
            b=node(view.leaf,d,c.L);bc=c.at(b);inds=tuple(sorted(selection[b]));cts={j:q.take(c.W) for j in inds}
            proof={p:q.take(TAG) for p in witnesses(bc.n,inds)}
            computed=local_open_root(bc,self.P,b,cts,proof)
    ...
    def write(self,view:View,changes:dict[int,tuple],stage:str):
        ...
        for d in ds:
            ...
            serialized[b]=hd+(b''.join(slots) if slots is not None else b'')+(b''.join(local[p] for p in local_indices(c.at(b).n)) if slots is not None and c.ring else b'')+bt
        ...
        out=self.io.rpc(c,WRITE,body,stage,{'leaf':view.leaf,'depths':list(ds),'full_depths':full},
                        {'headers_upload':sum(c.at(b).H for b in changes),'data_upload':dataup,'authentication_upload':authup})
    ...
    def subset(self,b:int,h:Header)->tuple[int,...]:
        c=self.c.at(b);real=set(h.live);unused=[j for j in range(c.n) if not(h.used>>j&1)];dummy=[j for j in unused if j not in real]
    ...
    def fused_logical(self,view:View,address:int):
        from cb_fusion import fused_logical
    ...
    def placement(self,pool:dict[int,Record],view:View,gamma:int,constrained:bool):
        ...
        for d in reversed(view.selected):
            ...
            chosen=[r for r in sorted(pool.values(),key=lambda x:x.uid) if node(r.leaf,d,c.L)==b and (not constrained or d in gc(gamma,r.leaf,c.L))][:c.at(b).Z]
        ...
        if c.kind=='path':
            ...
            if c.ring:
                for d in ds:
                    ...
                    if view.headers[b].count==c.at(b).S:self.neutral(view,b)
                selections={}
                for d in ds:
                    ...
                    dummy=[j for j in range(c.at(b).n) if not(h.used>>j&1) and j not in h.live]
