...
from audit_ab_dummy_first import inputs,audit_record
from queued_core_batch import free_memory


def main():
    path=PACKAGE/'ab_dummy_first_periodic_plan.json';plan,kwargs=inputs(path.name)
    checks=PACKAGE/'results/ab_dummy_first_periodic_checks.json';init=PACKAGE/'results/ab_dummy_first_controlled_checks.json'
    ...
    state=PACKAGE/'results/ab_dummy_first_periodic_queue_state.json';rows=[]
    for s in plan['specs']:
        ...
        if not p.exists():
            ...
            cmd=[sys.executable,'-B','-X','utf8',str(Path(__file__).parent/'run_ab_dummy_first_periodic.py'),'--spec',str(sp)]
    ...
    save(PACKAGE/'results/ab_dummy_first_periodic_queue_completion.json',dict(status='passed',rows=rows,plan_sha256=sha(path)))
