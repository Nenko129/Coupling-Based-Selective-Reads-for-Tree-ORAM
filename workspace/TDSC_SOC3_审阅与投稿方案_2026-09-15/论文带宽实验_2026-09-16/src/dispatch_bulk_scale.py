...
from run_bulk_scale import identity as core_identity
...
def main():
    ...
    path=Path(args.queue);plan=json.loads(path.read_text(encoding='utf-8'));name=path.stem+'_bulk';results=[]
    ...
    for item in plan['items']:
        ...
        lp=PACKAGE/'results/logs'/f"{s['id']}_bulk.log";assert not lp.exists(),'Inspect prior job before retry'
        assert item['entry']=='core'
