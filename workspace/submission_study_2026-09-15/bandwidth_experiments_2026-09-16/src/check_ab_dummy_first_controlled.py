...
def main():
    ...
    planpath=PACKAGE/'ab_dummy_first_controlled_check_plan.json';plan=dict(specs=specs,source_hashes=source,
    ...
    for s in specs:
        path=PACKAGE/'results/ab_dummy_first_controlled'/f"{s['layout']}_{s['kind']}.json"
        if path.exists():
            r=json.loads(path.read_text());assert r['status']=='passed' and r['spec']==s and r['source_hashes']==source;rows.append(r);continue
        phase='initializing';initial=None;c=None;b=None
        try:
            ...
            save(PACKAGE/'results/ab_dummy_first_controlled_progress.json',dict(status='partial',cases=rows,expected=6,source_hashes=source))
            ...
            save(PACKAGE/'results/ab_dummy_first_controlled_failure.json',dict(status='failed',spec=s,phase=phase,initialization=initial,
                error=traceback.format_exc(),source_hashes=source,
    ...
    save(PACKAGE/'results/ab_dummy_first_controlled_checks.json',dict(status='passed',source_hashes=source,cases=rows,plan_sha256=sha(planpath),
