"""One bounded-memory CB worker; do not hide failures or restart live work."""
from common import *
...
def main():
    path=PACKAGE/'ab_periodic_plan.json';plan,kwargs=inputs(path.name)
    proof=PACKAGE/'results/ab_periodic_checks.json'
    assert sha(proof)==plan['runner_checks_sha256'] and json.loads(proof.read_text())['source_hashes']==identity()
    state=PACKAGE/'results/ab_periodic_queue_state.json';rows=[]
    for s in plan['specs']:
        ...
        if not p.exists():
            ...
            cmd=[sys.executable,'-B','-X','utf8',str(Path(__file__).parent/'run_ab_periodic.py'),'--spec',str(sp)]
    ...
    save(PACKAGE/'results/ab_periodic_queue_completion.json',dict(status='passed',rows=rows,plan_sha256=sha(path)))
