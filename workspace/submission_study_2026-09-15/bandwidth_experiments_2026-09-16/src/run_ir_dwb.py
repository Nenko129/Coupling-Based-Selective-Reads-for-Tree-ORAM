...
def run(s):
    ...
    setup=b.io.snapshot();seq,trace=stored_trace(s['N'],s['warmup']+s['requests'],s['trace_seed'],s['workload'])
    ...
    row=dict(status='passed',spec=s,source_hashes=source,controller_proof_sha256=sha(proofpath),trace=trace,
        ...
        measurement='actual encrypted below-cache application frames at a fixed public slot horizon; dirty LLC lines may remain resident',
