# 最终证据链报告：SDE-GC-Path / R0-GC-Ring

日期：2026-09-11；版本 SOC2 v2。契约 SHA-256：`b40155e23d2d62cead87a89321cb88463ac0b36126782967fd490a813d41d19e`。
...
C=280、M=128，两种算术模式各输出8,711个height/threshold端点。完整重算得到：

- FE_UPWARD：H24(186)<=1.3052161448694381e-61。
- 逐运算NextUp：H24(186)<=1.3052161665818413e-61。
- 两者共同通过H24(186)<=2^-194。

已知状态表为未归一化subprobability上包络；cap外probability/first moment显式传播；最终取known stop-loss加unknown first moment；没有FFT、没有忽略小概率正状态。Poisson起点由Fraction Taylor界产生。160个小域精确有理比较通过，线程数1/4输出逐位一致。两种模式共享递推代码，不称为两个完全独立算法。
...
|---|---|---:|
| SDE | (190,142) | 1.0643440208594177376822026014340875994959649358699E-43 |
| R0 | (186,138) | 2.4135435754968873056704781639027050080374182611442E-42 |
...
按当前固定对象格式计算，两个外部服务器树的持久化**认证对象字节**约为SDE 264.946GiB、R0 471.898GiB；不含文件系统/对象索引/allocator开销。这也说明48GiB是逻辑payload容量而非物理服务器内存。本容器没有执行该大实例。
...
|---|---:|---:|---:|---:|---:|
| Path 同格式参照 | 1,250,048.00 | 27,664.00 | 7,168.00 | 368.00 | 1,285,248.00 |
| Deferred Full Read | 1,041,706.67 | 36,885.33 | 9,557.33 | 490.67 | 1,088,640.00 |
| SDE-GC-Path | 784,580.91 | 25,504.77 | 9,557.33 | 490.67 | 820,133.68 |
| Full-read Ring（lazy-neutral） | 884,515.38 | 37,385.90 | 41,562.51 | 1,394.70 | 964,858.49 |
| Full-GC Ring | 723,122.23 | 25,973.20 | 31,776.82 | 1,252.69 | 782,124.94 |
| R0-GC-Ring | 688,268.20 | 24,655.88 | 30,575.54 | 1,238.21 | 744,737.84 |
...
```text
q_F <= 9671406556917064594903040 < 2^84.
```

在相应时间、query count和input-length范围内明确假设每个PRF hybrid的epsilon_F<=2^-132，得到：SDE完整上界3.6755485343613672978776660840223859183823217947846E-40，R0为3.7216907178295862085157112420975718433440782301357E-40，二者都<2^-128。
...
- 双scalar grid各8,711个点；160个Fraction小域比较。
- 加密递归主回归2,700次顶层请求，其中小树差分oracle覆盖5,448次层访问（含该批setup）；另含4096B payload案例。
- 154,980项逐current-token深度关系，5,448项canonical packing对应，5,448项shadow field到三状态递推等式。
- 主回归24,933笔实际RPC账单逐项核对；另有六个同格式方案合计6,000次顶层请求的实执行账本（42,114笔RPC）。
- 静态field/class、priority count、commutation分别8,546项，root shift 4,273项；固定历史marking检查648个时刻。
