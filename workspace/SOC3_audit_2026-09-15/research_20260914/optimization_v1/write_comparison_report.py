...
d=json.loads((ROOT/'results/profile_comparison.json').read_text())
V={x['label']:x for x in d['variants']};F={x['label']:x for x in d['fair_baselines']}
assert all(x['capacity_plan']['hashes_checked'] for x in V.values())
fmt=lambda x:f'{x:,.2f}'
percent=lambda x:f'{x:.2f}%'
lines=['优化实施与基线对比：SDE-Path / R0-Ring，SOC3 研究变体',
       ...
       '', '| 原参照 | B/顶层请求 |', '|---|---:|']
for k,label in [('path','Path，Z4 同格式参照'),('ring','Full-read Ring，原 lazy-neutral 参照'),('sde','基础 SDE'),('r0','基础 R0')]:lines.append(f"| {label} | {fmt(d['original_bytes'][k])} |")
lines += ['', '| 新配置 | B/请求 | 比基础 SDE 少 | 比基础 R0 少 | 比原 Path 少 | 比原 Ring 少 |', '|---|---:|---:|---:|---:|---:|']
names=[('SDE-v3-512','SDE：512 B map＋紧凑 header'),('R0-v3-Z4-512','R0：保留 Z4/A3，512 B map＋紧凑 header＋融合'),('R0-v3-Z7-512','R0：Z7/A6、512 B map＋紧凑 header＋融合'),('R0-v3-Z7-best-with-path-budget','R0：Z7/A6、404 B map，含路径预算的搜索点')]
for key,label in names:
    x=V[key];r=x['reduction_vs_original_percent']
    lines.append(f"| {label} | {fmt(x['bytes'])} | {percent(r['sde'])} | {percent(r['r0'])} | {percent(r['path'])} | {percent(r['ring'])} |")
lines += ['', '对 SDE，原表相对 Path 的优势为 36.19%，本轮相对原 Path 扩大到 56.00%，增加约 19.81 个百分点。对 R0，原表相对 Ring 的优势为 22.81%，本轮相对原 Ring 扩大到 52.88%，增加约 30.06 个百分点。这里“百分点增加”与“相对自身降低 38.95%”是不同指标。',
...
for key,label in names[:3]:
    cs=V[key]['configs'];fields=['/'.join(str(c[f]) for c in cs) for f in ('B','Z','A','S','L','R')]
    lines.append('| '+label+' | '+' | '.join(fields)+' |')
lines += ['', '| 配置 | 持久 payload＋末级表 | 报告口径的路径 payload | 两者合计 | 服务器对象存储 |', '|---|---:|---:|---:|---:|',
          '| 原 SDE | 1,425,408 B | 397,312 B | 1,822,720 B | 264.95 GiB |',
          '| 原 R0 | 1,425,408 B | 380,928 B | 1,806,336 B | 471.90 GiB |']
for key,label in names:
    x=V[key];lines.append(f"| {label} | {x['payload_plus_terminal']:,} B | {x['report_path_payload_workspace']:,} B | {x['persistent_plus_report_path_payload']:,} B | {x['server_bytes']/2**30:.2f} GiB |")
lines += ['', '这是按原报告公式作的 payload 预留比较，不是 Python RSS 或完整总 RAM。header、descriptor、cached ciphertext、认证/加密 scratch、递归局部变量及运行时另计。Z7/A6 减少持久 stash，但更大的桶使数据路径 workspace 从 93 增至 155 个 4 KiB blocks；上表明确计入了这一变化。512 B 主配置的两项合计仍低于原 R0。',
...
fairpairs=[('SDE-v3-512','Path-Z4-common-opts-512','SDE vs Path，Z4'),('R0-v3-Z4-512','Ring-Z4A3-common-opts-512','R0 vs Ring，双方 Z4/A3'),('R0-v3-Z7-512','Ring-Z7A6-common-opts-512','R0 vs Ring，双方 Z7/A6'),('R0-v3-Z7-512','Ring-Z7A6/SDE-map-512','R0 vs Ring 数据树＋相同 SDE map 的混合参照')]
for vk,fk,label in fairpairs:
    x=V[vk];b=F[fk];lines.append(f"| {label} | {fmt(b['bytes'])} | {fmt(x['bytes'])} | {percent(x['reduction_vs_fair_percent'][fk])} |")
lines += ['', '因此，相对原表 Ring 的 52.88% 包含递归和通用报文优化的收益；在双方获得相同通用优化及 Z7/A6 后，仍保留约 23.26% 的通信优势。再给 Ring 使用本项目的 SDE 小块 map，R0 主配置仍少约 21.38%。',
...
for point in V['R0-v3-Z7-512']['capacity_plan']['points']:
    lines.append(f"| {point['tree']} | {point['height']} | {point['R']} | {point['threshold']} | {float(point['fixed_time_upper']):.10e} |")
plan=V['R0-v3-Z7-512']['capacity_plan']
lines += ['', f"包括 setup 的 lifetime stash 上界为 {float(plan['stash_lifetime_upper']):.10e}，低于原 R0 约 2.41354e-42。按 n≤13 重算采样误差、并使用更保守的 PRF 调用预算后，条件 ORAM advantage 上界约 {float(plan['conditional_advantage_upper']):.10e}，通过 2^-128 目标。",
