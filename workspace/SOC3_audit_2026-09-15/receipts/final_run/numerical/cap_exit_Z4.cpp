...
constexpr int Z=7;constexpr double rate=3.;
...
double ep=0.,em=0.;for(int j=0;j<=cap;j++){int u=cap+4-j+1;if(u>Tmax)continue;ep=add(ep,mul(f[j],tail[u]));em=add(em,mul(f[j],add(mul(double(cap+1),tail[u]),excess[u])));}
