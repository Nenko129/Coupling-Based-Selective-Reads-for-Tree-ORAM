...
def frame(op:int,tree:int,seq:int,body:bytes,response=False)->bytes:
    return b'SOC2'+bytes([2,op])+ui(int(response),2)+ui(tree,4)+ui(seq)+ui(len(body),8)+body

def parse_frame(raw:bytes):
    need(len(raw)>=FRAME and raw[:4]==b'SOC2' and raw[4]==2,'frame format')
...
class Config:
    kind:str; L:int; N:int; B:int=4096; Z:int=4; A:int=3; S:int=3; R:int=192; tree:int=0
    ...
    @property
    def secret(self):return ((8+64*self.Z+31)//32)*32
    @property
    def H(self):return PUBLIC+NONCE+self.secret
    @property
    def W(self):return self.B+NONCE
    def context(self):
        return encode(b'Selective-ORAM-chain-v2',self.kind.encode(),*[ui(v,8) for v in (self.tree,self.L,self.N,self.B,self.Z,self.A,self.S,self.R)])
...
class Header:
    ...
    def secret_bytes(self,c:Config)->bytes:
        ...
        raw=ui((1<<len(ds))-1,8)
        for d in ds:
            raw+=ui(d.uid)+ui(d.address,8)+ui(d.leaf,4)+ui(d.slot,2)+bytes(34)
        raw+=bytes(64*(c.Z-len(ds)))
