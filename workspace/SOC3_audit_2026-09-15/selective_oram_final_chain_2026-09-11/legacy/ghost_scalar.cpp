...
if(cap<4||cap>1000||height<1||height>64||M<4||M>512||threads<1||(mode!="up"&&mode!="next"))throw std::invalid_argument("guardrails");
next_mode=mode=="next";self_test();omp_set_num_threads(threads);
constexpr int Z=4;constexpr double rate=1.5;
...
set_rounding();p.swap(np);bp=nbp;bm=nbm;auto nf=totals(s,p);double mass=0.;for(double x:nf)mass=add(mass,x);double fm=add(moment(nf),bm);
